...
`border_thickness_px` is 1–256 (default 7). `event_line_thickness_px` is 1 through the border thickness and defaults to 3, or to the border thickness when that is smaller; it is stored for a future event line and does not change the current black border. `corner_radius_px` is 0–256 (default 0).
